...
async fn settlement_split_test() -> anyhow::Result<()> {
    ...
    // Escrow note storage — must match the contract's `get_storage()` layout.
    ...
    // Execute: consumer consumes the escrow note, which emits the two P2ID outputs.
    let tx = mock_chain
        .build_tx_context(consumer.clone(), &[escrow_note.id()], &[])?
        .build()?;
    let executed = tx.execute().await?;

    // Expected P2ID recipient digests.
