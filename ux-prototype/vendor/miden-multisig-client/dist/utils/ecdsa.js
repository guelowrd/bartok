...
export class EcdsaFormat {
    static normalizeSignatureHex(signatureHex) {
        ...
        throw new Error(`Invalid ECDSA signature length: expected ${ECDSA_SIGNATURE_BYTE_LENGTH} or ${ECDSA_SIGNATURE_WITH_RECOVERY_BYTE_LENGTH} bytes, got ${byteLength}`);
    }
    static normalizeRecoveryByte(signatureHex) {
        const clean = ensureHexPrefix(signatureHex).slice(2);
        const byteLength = clean.length / 2;
        if (byteLength === ECDSA_SIGNATURE_WITH_RECOVERY_BYTE_LENGTH) {
