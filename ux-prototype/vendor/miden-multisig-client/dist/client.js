...
export class MultisigClient {
    ...
    midenRpcEndpoint;
    _guardianClient;
    constructor(midenClient, config = {}) {
    ...
    async recoverByKey(signer) {
        ...
        return Promise.all(lookup.accounts.map(async ({ accountId }) => ({
            accountId,
     ...
     */
    async create(config, signer) {
        this._guardianClient.setSigner(signer);
        const { account } = await createMultisigAccount(this.midenClient, config, this.midenRpcEndpoint);
        const accountId = account.id().toString();
        await bindSignerAccountKey(signer, this.midenClient, accountId);
        return new Multisig(account, config, this._guardianClient, signer, this.midenClient, undefined, this.midenRpcEndpoint);
    }
    /**
    ...
    async load(accountId, signer) {
        ...
        await bindSignerAccountKey(signer, this.midenClient, accountId);
        return new Multisig(account, config, this._guardianClient, signer, this.midenClient, accountId, this.midenRpcEndpoint);
    }
}
