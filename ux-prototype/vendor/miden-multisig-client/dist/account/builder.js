 ...
 */
import { AccountBuilder, AccountComponent, AccountStorageMode, } from '@miden-sdk/miden-sdk';
import { getRawMidenClient } from '../raw-client.js';
import { buildMultisigStorageSlots, buildGuardianStorageSlots } from './storage.js';
import { MULTISIG_ECDSA_MASM, MULTISIG_MASM, GUARDIAN_ECDSA_MASM, GUARDIAN_MASM, } from './masm/auth.js';
import { MULTISIG_GUARDIAN_ACCOUNT_COMPONENT_MASM, MULTISIG_GUARDIAN_ECDSA_ACCOUNT_COMPONENT_MASM, } from './masm/account-components/auth.js';
 ...
 */
export async function createMultisigAccount(midenClient, config, midenRpcEndpoint) {
    ...
    if (config.procedureThresholds) {
        ...
        for (const pt of config.procedureThresholds) {
            ...
            if (pt.threshold > config.signerCommitments.length) {
                throw new Error(`procedure threshold (${pt.threshold}) cannot exceed number of signers (${config.signerCommitments.length})`);
            }
