...
export class ProposalFactory {
    ...
    assertCommitmentMatchesTxSummary(commitment, txSummaryBase64, context) {
        ...
        if (actualCommitment !== expectedCommitment) {
            throw new Error(`${context}: commitment ${actualCommitment} does not match tx_summary ${expectedCommitment}`);
        }
        return expectedCommitment;
    }
    fromDelta(delta, proposalId, metadata, existingSignatures = []) {
        this.assertAccountId(delta.accountId);
        const normalizedProposalId = this.assertCommitmentMatchesTxSummary(proposalId, delta.deltaPayload.txSummary.data, 'Invalid proposal');
        const resolvedMetadata = ProposalMetadataCodec.validate(metadata ?? ProposalMetadataCodec.fromGuardian(delta.deltaPayload.metadata));
        const localSignatures = new ProposalSignatures(existingSignatures, this.options.signerCommitments, `Invalid local proposal signatures for ${normalizedProposalId}`).entries();
        const serverSignatures = delta.status.status === 'pending'
            ? new ProposalSignatures(delta.status.cosignerSigs.map((signature) => ({
                signerId: signature.signerId,
                signature: signature.signature,
                timestamp: signature.timestamp,
            })), this.options.signerCommitments, `Invalid server proposal signatures for ${normalizedProposalId}`).entries()
            : [];
    ...
    fromExported(exported) {
        ...
        const normalizedCommitment = this.assertCommitmentMatchesTxSummary(exported.commitment, exported.txSummaryBase64, 'Invalid proposal');
        const metadata = ProposalMetadataCodec.validate(exported.metadata);
        const signatures = new ProposalSignatures(exported.signatures.map((signature) => {
            const scheme = signature.scheme ?? 'falcon';
            if (scheme === 'ecdsa' && !signature.publicKey) {
                throw new Error(`Invalid imported proposal signatures: ECDSA signature for ${signature.commitment} is missing publicKey`);
            }
            return {
        ...
        }), this.options.signerCommitments, 'Invalid imported proposal signatures').entries();
    ...
    toStatus(status, proposalType, signatures) {
