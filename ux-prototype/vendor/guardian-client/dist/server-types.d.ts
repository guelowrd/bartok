...
export type ServerDeltaStatus = {
    ...
    timestamp: string;
    proposer_id: string;
    cosigner_sigs: ServerCosignerSignature[];
} | {
    status: 'candidate';
    timestamp: string;
} | {
...
export type ServerProposalType = 'add_signer' | 'remove_signer' | 'change_threshold' | 'update_procedure_threshold' | 'switch_guardian' | 'consume_notes' | 'p2id' | 'custom' | (string & {});
